...
export const businessAdminService = {
  ...
  createService(id: string, draft: ServiceDraft) { return apiClient(`/businesses/${id}/services`, auth("POST", { ...draft, price: draft.price || null, currency: draft.price ? draft.currency : null, duration_minutes: draft.duration_minutes ? Number(draft.duration_minutes) : null, category_id: draft.category_id || null, platform_category_id: draft.platform_category_id || null, description: nullable(draft.description), image_url: nullable(draft.image_url) })); },
