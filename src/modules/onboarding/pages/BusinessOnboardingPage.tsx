...
export function BusinessOnboardingPage() {
  ...
  return <main className="onboarding-page"><header><button className="onboarding-back" onClick={() => router.back()} type="button">← Volver</button><Link href="/">El Bisne</Link><Link href="/login">Iniciar sesión</Link></header><div className="onboarding-progress"><span>Paso {step} de 3</span><div>{[1,2,3].map((item) => <i className={item <= step ? "active" : ""} key={item} />)}</div></div><section className="onboarding-card">
